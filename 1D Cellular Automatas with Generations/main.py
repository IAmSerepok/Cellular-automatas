...
class App:
    def __init__(
            self, columns=400, rows=400, tile_size=2, fps=60, speed=1,
    ...
    def set_rule(self, rule, generations):
        numb = rule
        rules = dict()
        for i in prange(8):
            value = numb % 2
    ...
    def check_cell(self, x, y):
        value = self.current_field[x, y]
        if value > 1:
            return (self.current_field[x, y] + 1) % self.generations
        else:
            a, c = x - 1, x + 1
            ...
            for state in [self.current_field[a, y], value, self.current_field[c, y]]:
                if state == 1:
                    s += '1'
                else:
                    s += '0'

            result = self.rules[int(s, 2)]

            if (value == 1) and (result == 0):
                return 2 % self.generations
